...
- [Complete Configuration Reference](#complete-configuration-reference) -- `info`, `colors`, `fonts`, `page`, `code`, `render`, `headings`, `outline`, `front-matter`, top-level keys
- [Themes and Presets](#themes-and-presets)
- [Front Matter](#front-matter)
- [Page Numbering](#page-numbering)
- [Cover Page Customization](#cover-page-customization)
...
- [Heading Variants](#heading-variants)
- [Cross-References](#cross-references)
- [Custom Outline](#custom-outline)
- [Code Blocks](#code-blocks)
- [Components](#components)
- [Development](#development) -- `just` recipes, running the tests
- [Migrating from 1.x](#migrating-from-1x)

## Dependencies

Pulled from Typst Universe on the first compile:
...
├── tests/                 # Test suite (tytanic, or plain `typst compile`)
├── template/
...
|-----|-----------|----------|
| `cover` | `(cfg) => content` | The cover page content. Not the page break that follows it, nor the front matter. |
...
The split is deliberate: bookkeeping stays with the template, appearance goes to the hook. A replacement chapter page never has to remember to break the page or to fix the heading counter, and a replacement cover never has to remember the front matter.
...
| Command | Effect |
|---------|--------|
| `just test` | Run everything through [tytanic](https://github.com/typst-community/tytanic) (binary `tt`) |
| `tt run front-matter` | Run a single test |
| `tt update test1` | Accept the current output as the new reference image |

Without tytanic installed, `typst` alone is enough:
